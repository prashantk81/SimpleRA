...
/**
 * @brief It removes a page from the deque if exists
 * 
*/
void BufferManager::eraseFromPool(string pageName) {
    logger.log("BufferManager::eraseFromPool");
    if(this->inPool(pageName)) {
        // Delete the page from the pool
...
void BufferManager::writePage(string tableName, int pageIndex, vector<vector<int>> rows, int rowCount)
{
    ...
    page.writePage(false);
}

void BufferManager::writePage(string tableName, int pageIndex, vector<vector<int>> rows, int rowIdx, int colIdx)
{
    logger.log("BufferManager::writePage");
    Page page(tableName, pageIndex, rows, rowIdx, colIdx);
    page.writePage(true);
}
