...
/**
 * @brief Get all the row from page
 *
 * @param
...
Page::Page(string tableName, int pageIndex, vector<vector<int>> rows, int rowIdx, int colIdx)
{
    logger.log("Page::Page");
    this->tableName = tableName;
    this->pageIndex = pageIndex;
    ...
    this->columnCount = rows[0].size();
    this->pageName = "../data/temp/" + this->tableName + "_Page" + "_R" + to_string(rowIdx) + "_C" + to_string(colIdx);
}
/**
 * @brief writes current page contents to file.
 *
 */
void Page::writePage(bool isMatrix)
{
...
/**
 * @brief Updates the content of a page
 * 
*/
void Page::updatePage(vector<vector<int> > updatedRows) {
    logger.log("Page::updatePage");
    this->rows.clear();
    this->rows = updatedRows;
    this->rowCount = updatedRows.size();
    this->columnCount = updatedRows[0].size();
